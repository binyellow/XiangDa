...
import "../grade/index.less";
import Taro from "@tarojs/taro";
import useHeader from "@/hooks/useHeader";
import useHistory from "@/hooks/useHistory";
import { ResultRef } from "@/pages/xingce/grade/components/Result";

const Grade = () => {
  const params = Taro.getCurrentInstance()?.router?.params;
  const total = params?.number || 10;
  const canvasRef = useRef<CanvasDrawingRef>(null);
  const [fenzi, setFenzi] = useState(getRandomInt(101, 1000));
  const [fenmu, setFenmu] = useState(getRandomInt(100, fenzi));
